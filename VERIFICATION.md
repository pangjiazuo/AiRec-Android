...
## 2026-09-15：通道回放 1.0.3

左侧时间轴、右侧事件截图，新增类型筛选和前后事件切换，密集事件可展开。已对照确认设计检查浅色、深色、横屏和竖屏运行截图。

50 项单元测试、Lint、8 项界面与时间轴检查通过。界面测试使用 Android 13 / API 33 模拟器及无人物素材；不代表 Android 12 真机已验收。截图保存在本地 `dist/ui-1.0.3/`，不会进入公开仓库。

配套主机 1.0.6 已覆盖安装到开发板，全天截图索引、存储测试和录像恢复通过；状态接口报告 AHD1 录像约 25 帧、预览约 16 帧。尚未验证五路同时实拍。

## 1.0.4：回放按钮

“概览”“精细”“返回实时”增加背景，倍速改为底部滑动条（0.5～2 倍），移除播放键左右的事件切换按钮。50 项单元测试、Lint 和深浅色各一项交互测试通过，已检查实际截图。截图位于本地 `dist/ui-1.0.4/`。
