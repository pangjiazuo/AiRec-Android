...
当前工程为 **1.4.0**，最低支持 Android 12。测试覆盖手机、平板尺寸、横竖屏、实时页面和录像回放。

- 已验证全天时间轴、重复定位、暂停后旋转及返回实时画面。
- 已验证详情页没有下载入口，全局录像列表保留下载。
- 实际运行测试使用 API 33 模拟器；不等于已经完成 Android 12 真机验证。
- 尚未完成五路真实摄像头同时在线的手机性能验收。

最新结果见 [1.4.0 时间轴测试](docs/VERIFICATION-TIMELINE-20260908.md)。下方是旧版本历史记录，包含当时的安装包与测试环境。
